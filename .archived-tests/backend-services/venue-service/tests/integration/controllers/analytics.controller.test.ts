 ...
 * 
 * Tests analytics proxy to external analytics service.
 * Returns 503 when analytics service is unavailable.
 */
...
describe('Analytics Controller Integration Tests', () => {
  ...
  describe('Proxy to Analytics Service', () => {
    it('should return 503 when analytics service is unavailable', async () => {
      const response = await context.app.inject({
        method: 'GET',
        url: `/api/v1/venues/${testVenueId}/analytics/summary`,
        headers: { authorization: `Bearer ${authToken}` }
      });

      // Analytics service is not running in test, should return 503
    ...
    it('should proxy POST requests and return 503', async () => {
      const response = await context.app.inject({
        method: 'POST',
        url: `/api/v1/venues/${testVenueId}/analytics/events`,
        headers: {
    ...
    it('should forward query parameters and return 503', async () => {
      const response = await context.app.inject({
        method: 'GET',
        url: `/api/v1/venues/${testVenueId}/analytics/reports?startDate=2024-01-01&endDate=2024-12-31`,
        headers: { authorization: `Bearer ${authToken}` }
      });
