...
class AuditService {
    ...
    async initializeTable() {
        ...
        CREATE TABLE IF NOT EXISTS audit_logs (
          ...
          INDEX idx_audit_service (service, timestamp DESC),
    ...
    async logAdminAction(service, action, userId, resourceType, details = {}) {
    ...
    async cleanup(retentionDays = 90) {
        ...
        const result = await this.pool.query('DELETE FROM audit_logs WHERE timestamp < $1', [cutoffDate]);
        return result.rowCount || 0;
    }
}
exports.AuditService = AuditService;
exports.auditService = AuditService.getInstance();
function auditMiddleware(service) {
    return async (req, res, next) => {
        ...
        res.on('finish', async () => {
            const shouldAudit = req.path.includes('/admin') ||
                req.method === 'POST' ||
                req.method === 'PUT' ||
                req.method === 'PATCH' ||
                req.method === 'DELETE';
            if (shouldAudit && req.user) {
                ...
                await exports.auditService.logAction({
                    service,
                    action: `${req.method} ${req.path}`,
                    actionType: req.method === 'DELETE' ? 'DELETE' :
                        req.method === 'POST' ? 'CREATE' :
                            req.method === 'GET' ? 'ACCESS' : 'UPDATE',
