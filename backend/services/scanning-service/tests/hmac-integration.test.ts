// @ts-nocheck
/**
 * HMAC Integration Tests - scanning-service
 *
 * Tests for HMAC authentication on service-to-service communication.
 * NOTE: scanning-service only receives S2S calls, it doesn't make outbound calls.
 */
...
process.env.SERVICE_NAME = 'scanning-service';
...
describe('HMAC Integration Tests - scanning-service', () => {
  ...
  beforeAll(() => {
    ...
    // Create signers for services that call scanning-service
    ticketServiceSigner = createHmacSigner({
      secret: testSecret,
      serviceName: 'ticket-service',
    });

    eventServiceSigner = createHmacSigner({
      secret: testSecret,
  ...
  // =========================================================================
  // OUTGOING CALLS - Headers Generation (for completeness)
  // Note: scanning-service doesn't typically make outbound S2S calls
  // =========================================================================
  ...
  describe('Outgoing calls include HMAC headers', () => {
    test('should generate all required HMAC headers for GET request', () => {
      const signer = createHmacSigner({
        secret: testSecret,
        serviceName: 'scanning-service',
      });
      const headers = signer.sign('GET', '/api/v1/devices/device-123');

      expect(headers[HMAC_HEADER_NAMES.SERVICE]).toBe('scanning-service');
    ...
    test('should generate body hash header for POST request with body', () => {
      ...
      const body = { ticketId: 'ticket-123', scanResult: 'VALID' };
      const headers = signer.sign('POST', '/api/v1/scans', body);
    ...
    test('should generate unique nonce for each request', () => {
      ...
      const headers1 = signer.sign('GET', '/api/v1/policies');
      const headers2 = signer.sign('GET', '/api/v1/policies');
  ...
  // Primary focus: scanning-service receives calls from other services
  // =========================================================================

  describe('Incoming calls validate HMAC', () => {
    test('should validate request from ticket-service', async () => {
      const method = 'GET';
      const path = '/api/v1/internal/scan-status/ticket-123';
    ...
    test('should validate POST request from event-service with body', async () => {
      const method = 'POST';
      const path = '/api/v1/internal/policies/sync';
      const body = { eventId: 'event-123', policy: { maxScans: 1 } };
      const headers = toMutableHeaders(eventServiceSigner.sign(method, path, body));
    ...
    test('should extract correct service name from different services', async () => {
      ...
      const headers = toMutableHeaders(orderServiceSigner.sign('GET', '/api/v1/internal/check'));
      const result = await validator.validate(headers, 'GET', '/api/v1/internal/check');
  ...
  describe('Security - Replay attack prevention', () => {
    test('should reject request with reused nonce', async () => {
      const method = 'POST';
      const path = '/api/v1/internal/scan';
      const headers = toMutableHeaders(ticketServiceSigner.sign(method, path));
  ...
  describe('Security - Invalid signature detection', () => {
    test('should reject request with tampered signature', async () => {
      const method = 'POST';
      const path = '/api/v1/internal/scan-tampered';
      const headers = toMutableHeaders(ticketServiceSigner.sign(method, path));
    ...
    test('should reject request signed with wrong secret', async () => {
      ...
      const headers = toMutableHeaders(wrongSigner.sign('POST', '/api/v1/internal/scan'));
      const result = await validator.validate(headers, 'POST', '/api/v1/internal/scan');
  ...
  describe('Security - Missing headers detection', () => {
    test('should reject request missing service header', () => {
      const headers = toMutableHeaders(ticketServiceSigner.sign('POST', '/api/v1/internal/scan'));
