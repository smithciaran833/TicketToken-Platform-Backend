/**
 * Internal Auth Middleware - scanning-service
 *
 * Validates HMAC-authenticated requests from other internal services
 * using the shared library's standardized validation.
 *
 * NOTE: This service's QR HMAC (for ticket validation) is separate from
 * service-to-service HMAC. The QR HMAC uses HMAC_SECRET env var and validates
 * ticket QR codes. The internal auth uses INTERNAL_HMAC_SECRET for S2S calls.
 */
...
// NOTE: INTERNAL_HMAC_SECRET is for S2S auth, HMAC_SECRET is for QR codes
...
const ALLOWED_SERVICES = new Set(
  (process.env.ALLOWED_INTERNAL_SERVICES || 'ticket-service,event-service,order-service,venue-service')
    .split(',')
