...
export default class OfflineCache {
  private cacheWindowMinutes: number;
   ...
   */
  async generateEventCache(eventId: string): Promise<CacheResult> {
    const pool = getPool();
   ...
   */
  async getDeviceCache(deviceId: string, eventId: string): Promise<DeviceCacheResult> {
    const pool = getPool();
   ...
   */
  async validateOfflineScan(ticketId: string, validationHash: string, eventId: string): Promise<ValidationResult> {
    const pool = getPool();
