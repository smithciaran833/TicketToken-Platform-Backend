...
export async function up(knex: Knex): Promise<void> {
  // 1. QUEUES TABLE
  await knex.schema.createTable('queues', (table: any) => {
  ...
  // 2. JOBS TABLE
  await knex.schema.createTable('jobs', (table: any) => {
    table.uuid('id').primary().defaultTo(knex.raw('gen_random_uuid()'));
  ...
  // 3. SCHEDULES TABLE
  await knex.schema.createTable('schedules', (table: any) => {
    table.uuid('id').primary().defaultTo(knex.raw('gen_random_uuid()'));
    table.string('name', 255).notNullable().unique();
  ...
  // 4. RATE_LIMITS TABLE
  await knex.schema.createTable('rate_limits', (table: any) => {
    table.uuid('id').primary().defaultTo(knex.raw('gen_random_uuid()'));
  ...
  await knex.schema.createTable('critical_jobs', (table: any) => {
    table.uuid('id').primary().defaultTo(knex.raw('gen_random_uuid()'));
  ...
  await knex.schema.createTable('queue_metrics', (table: any) => {
    table.uuid('id').primary().defaultTo(knex.raw('gen_random_uuid()'));
    table.string('queue_name', 255).notNullable();
  ...
  await knex.schema.createTable('idempotency_keys', (table: any) => {
    table.uuid('id').primary().defaultTo(knex.raw('gen_random_uuid()'));
    table.string('key', 255).notNullable().unique();
    table.string('queue_name', 255).notNullable();
    table.string('job_type', 255).notNullable();
  ...
  await knex.schema.createTable('rate_limiters', (table: any) => {
  ...
  await knex.schema.createTable('alert_history', (table: any) => {
    table.uuid('id').primary().defaultTo(knex.raw('gen_random_uuid()'));
  ...
  await knex.schema.createTable('dead_letter_jobs', (table: any) => {
    table.uuid('id').primary().defaultTo(knex.raw('gen_random_uuid()'));
    table.string('queue_name', 255).notNullable();
    table.string('job_type', 255).notNullable();
    table.jsonb('job_data').defaultTo('{}');
