// @ts-nocheck
/**
 * HMAC Integration Tests - queue-service
 *
...
describe('HMAC Integration Tests - queue-service', () => {
  ...
  describe('Outgoing calls include HMAC headers', () => {
    test('should generate all required HMAC headers for GET request', () => {
      ...
      expect(headers[HMAC_HEADER_NAMES.SERVICE]).toBe('queue-service');
    ...
    test('should generate body hash header for POST request with body', () => {
      const body = { userId: 'user-123', amount: 100, currency: 'USD' };
      const headers = signer.sign('POST', '/api/v1/payments/process', body);
    ...
    test('should generate unique nonce for each request', () => {
      const headers1 = signer.sign('POST', '/api/v1/mint');
      const headers2 = signer.sign('POST', '/api/v1/mint');
  ...
  describe('Incoming calls validate HMAC', () => {
    ...
    test('should extract service name from validated request', async () => {
      ...
      const headers = toMutableHeaders(orderServiceSigner.sign('POST', '/api/v1/internal/jobs'));
      const result = await validator.validate(headers, 'POST', '/api/v1/internal/jobs');
  ...
  describe('Security - Replay attack prevention', () => {
    test('should reject request with reused nonce', async () => {
      const method = 'POST';
      const path = '/api/v1/internal/jobs/secure';
      const headers = toMutableHeaders(signer.sign(method, path));
  ...
  describe('Security - Invalid signature detection', () => {
    test('should reject request with tampered signature', async () => {
      const method = 'POST';
      const path = '/api/v1/internal/jobs/tampered';
      const headers = toMutableHeaders(signer.sign(method, path));
    ...
    test('should reject request signed with wrong secret', async () => {
      ...
      const headers = toMutableHeaders(wrongSigner.sign('POST', '/api/v1/internal/jobs'));
      const result = await validator.validate(headers, 'POST', '/api/v1/internal/jobs');
  ...
  describe('Security - Missing headers detection', () => {
    test('should reject request missing service header', () => {
      const headers = toMutableHeaders(signer.sign('POST', '/api/v1/internal/jobs'));
