...
describe('OAuthService Integration Tests', () => {
  ...
  async function createOAuthConnection(userId: string, provider: string, providerUserId: string) {
    await testPool.query(
      ...
      [crypto.randomUUID(), userId, provider, providerUserId, JSON.stringify({ id: providerUserId })]
    );
  }
  ...
  describe('createSession (via database operations)', () => {
    it('should create user session', async () => {
      ...
      await testPool.query(
        ...
        [sessionId, user.id, '192.168.1.1', 'OAuth Test Agent', JSON.stringify({})]
      );
    ...
    it('should enforce unique provider + provider_user_id', async () => {
      ...
      await expect(
        testPool.query(
          `INSERT INTO oauth_connections (id, user_id, provider, provider_user_id, profile_data, created_at, updated_at)
           VALUES ($1, $2, $3, $4, $5, NOW(), NOW())`,
          [crypto.randomUUID(), user2.id, 'google', 'same-google-id', JSON.stringify({})]
        )
      ).rejects.toThrow();
    });
  ...
  describe('OAuth profile data storage', () => {
    it('should store profile data as JSON', async () => {
      ...
      await testPool.query(
        `INSERT INTO oauth_connections (id, user_id, provider, provider_user_id, profile_data, created_at, updated_at)
         VALUES ($1, $2, $3, $4, $5, NOW(), NOW())`,
        [crypto.randomUUID(), user.id, 'google', 'google-with-profile', JSON.stringify(profileData)]
      );
    ...
    it('should update profile data on subsequent logins', async () => {
      ...
      await testPool.query(
        `INSERT INTO oauth_connections (id, user_id, provider, provider_user_id, profile_data, created_at, updated_at)
         VALUES ($1, $2, $3, $4, $5, NOW(), NOW())`,
        [crypto.randomUUID(), user.id, 'google', 'google-update', JSON.stringify({ name: 'Old Name' })]
      );
