...
describe('RBACService Integration Tests', () => {
  ...
  async function createVenueRole(
    ...
    await testPool.query(
      `INSERT INTO user_venue_roles (id, user_id, venue_id, role, granted_by, expires_at, is_active, granted_at)
       VALUES ($1, $2, $3, $4, $5, $6, $7, NOW())`,
      [crypto.randomUUID(), userId, venueId, role, grantedBy, expiresAt, isActive]
    );
  }
  ...
  describe('getUserPermissions', () => {
    ...
    it('should include venue role permissions when venueId provided', async () => {
      ...
      const permissions = await rbacService.getUserPermissions(user.id, TEST_VENUE_ID);
    ...
    it('should not return permissions for different venue', async () => {
      ...
      const permissions = await rbacService.getUserPermissions(user.id, otherVenueId);
  ...
  describe('checkPermission', () => {
    it('should return true for permission user has', async () => {
      ...
      const hasPermission = await rbacService.checkPermission(user.id, 'tickets:purchase');
    ...
    it('should return true for any permission when user has wildcard', async () => {
      ...
      const hasPermission = await rbacService.checkPermission(user.id, 'any:permission', TEST_VENUE_ID);
    ...
    it('should check venue-scoped permission correctly', async () => {
      ...
      const hasPermissionForVenue = await rbacService.checkPermission(user.id, 'tickets:sell', TEST_VENUE_ID);
      const hasPermissionGlobally = await rbacService.checkPermission(user.id, 'tickets:sell');
  ...
  describe('requirePermission', () => {
    it('should not throw when user has permission', async () => {
      ...
      await expect(
        rbacService.requirePermission(user.id, 'tickets:purchase')
    ...
    it('should throw AuthorizationError when user lacks permission', async () => {
      ...
      await expect(
        rbacService.requirePermission(user.id, 'events:create')
    ...
    it('should not throw when venue-owner checks any permission for their venue', async () => {
      ...
      await expect(
        rbacService.requirePermission(user.id, 'anything:at:all', TEST_VENUE_ID)
      ).resolves.not.toThrow();
    });
  });
  ...
  describe('grantVenueRole', () => {
    it('should grant role to user', async () => {
      ...
      await rbacService.grantVenueRole(user.id, TEST_VENUE_ID, 'box-office', admin.id);
    ...
    it('should set expiration date when provided', async () => {
      ...
      await rbacService.grantVenueRole(user.id, TEST_VENUE_ID, 'door-staff', admin.id, expiresAt);
    ...
    it('should reject invalid role', async () => {
      ...
      await expect(
        rbacService.grantVenueRole(user.id, TEST_VENUE_ID, 'fake-role', admin.id)
    ...
    it('should reject granter without roles:manage permission', async () => {
      ...
      await expect(
        rbacService.grantVenueRole(user.id, TEST_VENUE_ID, 'box-office', notAdmin.id)
    ...
    it('should update expiration if role already exists', async () => {
      ...
      await rbacService.grantVenueRole(user.id, TEST_VENUE_ID, 'box-office', admin.id, newExpiration);
  ...
  describe('revokeVenueRole', () => {
    it('should deactivate role', async () => {
      ...
      await rbacService.revokeVenueRole(user.id, TEST_VENUE_ID, 'box-office', admin.id);
    ...
    it('should reject revoker without roles:manage permission', async () => {
      ...
      await expect(
        rbacService.revokeVenueRole(user.id, TEST_VENUE_ID, 'box-office', notAdmin.id)
      ).rejects.toThrow('Missing required permission: roles:manage');
    });
  ...
  describe('getUserVenueRoles', () => {
    it('should return all active roles for user', async () => {
      ...
      const roles = await rbacService.getUserVenueRoles(user.id);
