...
describe('ProfileController Integration Tests', () => {
    ...
    it('should export all user data', async () => {
      ...
      await testPool.query(
        `INSERT INTO user_sessions (user_id, ip_address, user_agent, started_at)
         VALUES ($1, '1.2.3.4', 'Browser', NOW())`,
        [user.id]
      );

      await testPool.query(
        `INSERT INTO user_addresses (user_id, address_type, address_line1, city, postal_code, country_code)
         VALUES ($1, 'billing', '123 Main St', 'City', '12345', 'US')`,
        [user.id]
      );
  ...
  describe('requestDeletion', () => {
    ...
    it('should revoke all sessions on deletion', async () => {
      ...
      // Create sessions
      await testPool.query(
        `INSERT INTO user_sessions (user_id, ip_address, started_at) VALUES ($1, '1.1.1.1', NOW())`,
        [user.id]
      );
      await testPool.query(
        `INSERT INTO user_sessions (user_id, ip_address, started_at) VALUES ($1, '2.2.2.2', NOW())`,
        [user.id]
      );
