...
describe('SessionController Integration Tests', () => {
  ...
  async function createSession(userId: string, overrides: Partial<any> = {}) {
    const result = await testPool.query(
      `INSERT INTO user_sessions (user_id, ip_address, user_agent, started_at, metadata)
       VALUES ($1, $2, $3, NOW(), $4)
       RETURNING *`,
      [
        userId,
  ...
  describe('listSessions', () => {
    ...
    it('should not return ended sessions', async () => {
      ...
      await testPool.query(
        `INSERT INTO user_sessions (user_id, ip_address, user_agent, started_at, ended_at)
         VALUES ($1, '192.168.1.99', 'Old Browser', NOW() - INTERVAL '1 day', NOW())`,
        [user.id]
      );
    ...
    it('should return sessions ordered by started_at DESC', async () => {
      ...
      await testPool.query(
        `INSERT INTO user_sessions (user_id, ip_address, user_agent, started_at)
         VALUES ($1, '192.168.1.1', 'Old', NOW() - INTERVAL '2 hours')`,
        [user.id]
      );
      await testPool.query(
        `INSERT INTO user_sessions (user_id, ip_address, user_agent, started_at)
         VALUES ($1, '192.168.1.2', 'New', NOW())`,
        [user.id]
      );
