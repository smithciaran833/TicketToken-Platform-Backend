...
describe('DeviceTrustService Integration Tests', () => {
  ...
  describe('calculateTrustScore', () => {
    it('should return 0 for unknown device', async () => {
      ...
      const score = await deviceTrustService.calculateTrustScore(user.id, fingerprint, TEST_TENANT_ID);
    ...
    it('should return base score of 50 for new known device', async () => {
      ...
      await testPool.query(
        `INSERT INTO trusted_devices (user_id, device_fingerprint, trust_score, last_seen, created_at, tenant_id)
         VALUES ($1, $2, 50, NOW(), NOW(), $3)`,
        [user.id, fingerprint, TEST_TENANT_ID]
      );
    ...
    it('should add age bonus for older devices', async () => {
      ...
      await testPool.query(
        `INSERT INTO trusted_devices (user_id, device_fingerprint, trust_score, last_seen, created_at, tenant_id)
         VALUES ($1, $2, 50, NOW(), NOW() - INTERVAL '30 days', $3)`,
        [user.id, fingerprint, TEST_TENANT_ID]
      );
    ...
    it('should cap age bonus at 20 points', async () => {
      ...
      await testPool.query(
        `INSERT INTO trusted_devices (user_id, device_fingerprint, trust_score, last_seen, created_at, tenant_id)
         VALUES ($1, $2, 50, NOW(), NOW() - INTERVAL '365 days', $3)`,
        [user.id, fingerprint, TEST_TENANT_ID]
      );
    ...
    it('should reduce activity bonus for older last_seen', async () => {
      ...
      await testPool.query(
        `INSERT INTO trusted_devices (user_id, device_fingerprint, trust_score, last_seen, created_at, tenant_id)
         VALUES ($1, $2, 50, NOW() - INTERVAL '10 days', NOW() - INTERVAL '30 days', $3)`,
        [user.id, fingerprint, TEST_TENANT_ID]
      );
    ...
    it('should give 20 points for device seen within a week', async () => {
      ...
      await testPool.query(
        `INSERT INTO trusted_devices (user_id, device_fingerprint, trust_score, last_seen, created_at, tenant_id)
         VALUES ($1, $2, 50, NOW() - INTERVAL '3 days', NOW(), $3)`,
        [user.id, fingerprint, TEST_TENANT_ID]
      );
    ...
    it('should cap total score at 100', async () => {
      ...
      await testPool.query(
        `INSERT INTO trusted_devices (user_id, device_fingerprint, trust_score, last_seen, created_at, tenant_id)
         VALUES ($1, $2, 100, NOW(), NOW() - INTERVAL '500 days', $3)`,
        [user.id, fingerprint, TEST_TENANT_ID]
      );
  ...
  describe('recordDeviceActivity', () => {
    it('should create new device record on first activity', async () => {
      ...
      await deviceTrustService.recordDeviceActivity(user.id, fingerprint, TEST_TENANT_ID, true);
    ...
    it('should create device with score 0 on failed first activity', async () => {
      ...
      await deviceTrustService.recordDeviceActivity(user.id, fingerprint, TEST_TENANT_ID, false);
    ...
    it('should increase score on successful activity', async () => {
      ...
      await testPool.query(
        `INSERT INTO trusted_devices (user_id, device_fingerprint, trust_score, last_seen, tenant_id)
         VALUES ($1, $2, 50, NOW(), $3)`,
        [user.id, fingerprint, TEST_TENANT_ID]
      );
      ...
      expect(device.rows[0].trust_score).toBe(55);
    });
    ...
    it('should decrease score on failed activity', async () => {
      ...
      expect(device.rows[0].trust_score).toBe(40);
    });
    ...
    it('should update last_seen timestamp', async () => {
      ...
      await testPool.query(
        `INSERT INTO trusted_devices (user_id, device_fingerprint, trust_score, last_seen, tenant_id)
         VALUES ($1, $2, 50, NOW() - INTERVAL '1 day', $3)`,
        [user.id, fingerprint, TEST_TENANT_ID]
      );
  ...
  describe('requiresAdditionalVerification', () => {
    it('should require verification for unknown device', async () => {
      ...
      const requires = await deviceTrustService.requiresAdditionalVerification(user.id, fingerprint, TEST_TENANT_ID);
    ...
    it('should require verification for low trust device', async () => {
      ...
      await testPool.query(
        `INSERT INTO trusted_devices (user_id, device_fingerprint, trust_score, last_seen, created_at, tenant_id)
         VALUES ($1, $2, 10, NOW() - INTERVAL '60 days', NOW() - INTERVAL '60 days', $3)`,
        [user.id, fingerprint, TEST_TENANT_ID]
      );
    ...
    it('should not require verification for trusted device', async () => {
      ...
      await testPool.query(
        `INSERT INTO trusted_devices (user_id, device_fingerprint, trust_score, last_seen, created_at, tenant_id)
         VALUES ($1, $2, 80, NOW(), NOW() - INTERVAL '30 days', $3)`,
        [user.id, fingerprint, TEST_TENANT_ID]
      );
