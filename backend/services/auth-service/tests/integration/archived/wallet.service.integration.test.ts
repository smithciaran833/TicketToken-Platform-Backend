...
describe('WalletService Integration Tests', () => {
  ...
  async function createWalletConnection(userId: string, walletAddress: string, network: string = 'solana') {
    await testPool.query(
      `INSERT INTO wallet_connections (id, user_id, wallet_address, network, verified, created_at, tenant_id)
       VALUES ($1, $2, $3, $4, true, NOW(), $5)`,
      [crypto.randomUUID(), userId, walletAddress, network, TEST_TENANT_ID]
    );
  }
  ...
  describe('wallet connection database operations', () => {
    it('should store wallet connection with correct fields', async () => {
      ...
      await testPool.query(
        `INSERT INTO wallet_connections (id, user_id, wallet_address, network, verified, created_at, last_login_at, tenant_id)
         VALUES ($1, $2, $3, $4, $5, NOW(), NOW(), $6)`,
        [crypto.randomUUID(), user.id, walletAddress, 'solana', true, TEST_TENANT_ID]
      );
    ...
    it('should not find unverified wallet', async () => {
      ...
      await testPool.query(
        ...
        [crypto.randomUUID(), user.id, walletAddress, 'solana', TEST_TENANT_ID]
      );
  ...
  describe('wallet registration database flow', () => {
    ...
    it('should create session on wallet registration', async () => {
      ...
      const sessionResult = await testPool.query(
        `INSERT INTO user_sessions (id, user_id, started_at, tenant_id)
         VALUES ($1, $2, NOW(), $3)
         RETURNING id`,
        [crypto.randomUUID(), user.id, TEST_TENANT_ID]
      );
    ...
    it('should allow same wallet for different networks', async () => {
      ...
      await expect(
        testPool.query(
          `INSERT INTO wallet_connections (id, user_id, wallet_address, network, verified, created_at, tenant_id)
           VALUES ($1, $2, $3, $4, true, NOW(), $5)`,
          [crypto.randomUUID(), user.id, walletAddress, 'ethereum', TEST_TENANT_ID]
        )
