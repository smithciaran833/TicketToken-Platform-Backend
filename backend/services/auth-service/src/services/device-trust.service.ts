...
export class DeviceTrustService {
   ...
   */
  async calculateTrustScore(userId: string, fingerprint: string, tenantId: string): Promise<number> {
    const device = await db('trusted_devices')
      .where({ user_id: userId, device_fingerprint: fingerprint, tenant_id: tenantId })
      .first();
   ...
   */
  async recordDeviceActivity(userId: string, fingerprint: string, tenantId: string, success: boolean): Promise<void> {
    const device = await db('trusted_devices')
      .where({ user_id: userId, device_fingerprint: fingerprint, tenant_id: tenantId })
      .first();
   ...
   */
  async requiresAdditionalVerification(userId: string, fingerprint: string, tenantId: string): Promise<boolean> {
    const score = await this.calculateTrustScore(userId, fingerprint, tenantId);
