// @ts-nocheck
/**
 * HMAC Integration Tests - venue-service
 *
...
process.env.SERVICE_NAME = 'venue-service';
...
describe('HMAC Integration Tests - venue-service', () => {
  ...
  describe('Outgoing calls include HMAC headers', () => {
    test('should generate all required HMAC headers for GET request', () => {
      const headers = signer.sign('GET', '/api/v1/events/by-venue');

      expect(headers[HMAC_HEADER_NAMES.SERVICE]).toBe('venue-service');
    ...
    test('should generate body hash header for POST request with body', () => {
      const body = { venueId: 'venue-123', name: 'Test Venue', location: 'NYC' };
      const headers = signer.sign('POST', '/api/v1/venues/register', body);
    ...
    test('should generate unique nonce for each request', () => {
      const headers1 = signer.sign('POST', '/api/v1/venues');
      const headers2 = signer.sign('POST', '/api/v1/venues');
  ...
  describe('Incoming calls validate HMAC', () => {
    ...
    test('should extract service name from validated request', async () => {
      ...
      const headers = toMutableHeaders(eventServiceSigner.sign('POST', '/api/v1/internal/venues'));
      const result = await validator.validate(headers, 'POST', '/api/v1/internal/venues');
  ...
  describe('Security - Replay attack prevention', () => {
    test('should reject request with reused nonce', async () => {
      const method = 'POST';
      const path = '/api/v1/internal/venues/secure';
      const headers = toMutableHeaders(signer.sign(method, path));
  ...
  describe('Security - Invalid signature detection', () => {
    test('should reject request with tampered signature', async () => {
      const method = 'POST';
      const path = '/api/v1/internal/venues/tampered';
      const headers = toMutableHeaders(signer.sign(method, path));
    ...
    test('should reject request signed with wrong secret', async () => {
      ...
      const headers = toMutableHeaders(wrongSigner.sign('POST', '/api/v1/internal/venues'));
      const result = await validator.validate(headers, 'POST', '/api/v1/internal/venues');
  ...
  describe('Security - Missing headers detection', () => {
    test('should reject request missing service header', () => {
      const headers = toMutableHeaders(signer.sign('POST', '/api/v1/internal/venues'));
