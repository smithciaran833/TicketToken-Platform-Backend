...
// AUDIT FIX #21: Use structured logger instead of console.log
// Migration logger - falls back to console if logger not available
const migrationLog = {
  info: (message: string) => {
    try {
      // eslint-disable-next-line @typescript-eslint/no-require-imports
      const { logger } = require('../utils/logger');
      logger.info({ component: 'migration' }, message);
    } catch {
  ...
  error: (message: string, error?: Error) => {
    try {
      // eslint-disable-next-line @typescript-eslint/no-require-imports
      const { logger } = require('../utils/logger');
      logger.error({ component: 'migration', err: error }, message);
    } catch {
      // eslint-disable-next-line no-console
...
/**
 * Marketplace Service Baseline Migration
 * 
 * CRITICAL: tenant_id has NO default value by design.
 * This enforces that all inserts MUST provide a tenant_id explicitly.
 * If tenant context fails silently, the insert will fail rather than
 * silently routing data to a default tenant (which could cause data leakage).
 * 
 * FIX #2: Removed dangerous default tenant_id to prevent multi-tenant data isolation failures.
 */
...
export async function up(knex: Knex): Promise<void> {
  ...
  migrationLog.info('🏪 Starting Marketplace Service baseline migration...');
  ...
  await knex.schema.createTableIfNotExists('marketplace_listings', (table) => {
    ...
    // FIX #2: No default - tenant_id MUST be explicitly provided
  ...
  migrationLog.info('✅ marketplace_listings table created');
  ...
  migrationLog.info('✅ marketplace_transfers table created');
  ...
  migrationLog.info('✅ platform_fees table created');
  ...
  migrationLog.info('✅ venue_marketplace_settings table created');
  ...
  migrationLog.info('✅ marketplace_price_history table created');
  ...
  migrationLog.info('✅ marketplace_disputes table created');
  ...
  migrationLog.info('✅ dispute_evidence table created');
  ...
  migrationLog.info('✅ tax_transactions table created');
  ...
  migrationLog.info('✅ anti_bot_activities table created');
  ...
  migrationLog.info('✅ anti_bot_violations table created');
  ...
  migrationLog.info('✅ marketplace_blacklist table created');
  ...
  migrationLog.info('✅ expire_marketplace_listings() function created');
  ...
  migrationLog.info('✅ calculate_marketplace_fees() function created');
  ...
  migrationLog.info('✅ get_user_active_listings_count() function created');
  ...
  migrationLog.info('✅ RLS enabled on all tables');

  // ==========================================
  // FOREIGN KEY CONSTRAINTS
  // ==========================================
  migrationLog.info('🔗 Adding foreign key constraints...');
  ...
  migrationLog.info('✅ All foreign key constraints added (7 internal + 22 cross-service = 29 total)');
  migrationLog.info('🎉 Marketplace Service baseline migration complete! 11 tables + 3 stored procedures + 29 FK constraints');
}
...
export async function down(knex: Knex): Promise<void> {
  ...
  migrationLog.info('✅ Marketplace Service migration rolled back');
}
