/**
 * Internal Service Authentication Middleware for Marketplace Service
 * 
 * Issues Fixed:
 * - S2S-1: No internal auth middleware → HMAC-SHA256 signature verification
 * - S2S-2: No service identity validation → Proper request validation
 * - S2S-6: No request ID propagation → Pass through X-Request-ID header
 */
...
// Allowed internal services that can call marketplace-service
...
// HMAC replay window - 60 seconds to prevent replay attacks
 ...
 * Use this when marketplace-service calls other services
 */
...
export function buildInternalHeaders(body: any, requestId?: string): Record<string, string> {
  ...
  // S2S-6: Propagate request ID for distributed tracing
  if (requestId) {
...
export function createInternalFetch(baseUrl: string) {
  return async (path: string, options: RequestInit = {}, requestId?: string): Promise<Response> => {
