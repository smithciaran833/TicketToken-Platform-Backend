// @ts-nocheck
/**
 * HMAC Integration Tests - payment-service
 *
 * Tests for HMAC authentication on service-to-service communication.
 * Note: Uses real timers to avoid conflicts with setup.ts fake timers.
 */
...
describe('HMAC Integration Tests - payment-service', () => {
  ...
  beforeAll(() => {
    // Use real timers for HMAC tests (override setup.ts fake timers)
    jest.useRealTimers();
  ...
  beforeEach(() => {
    // Ensure real timers for each test
    jest.useRealTimers();
  ...
  describe('Outgoing calls include HMAC headers', () => {
    test('should generate all required HMAC headers for GET request', () => {
      const headers = signer.sign('GET', '/api/v1/payments/status');

      expect(headers[HMAC_HEADER_NAMES.SERVICE]).toBe('payment-service');
    ...
    test('should generate body hash header for POST request with body', () => {
      const body = { userId: 'user-123', amount: 10000, currency: 'usd' };
      const headers = signer.sign('POST', '/api/v1/payments/create', body);
    ...
    test('should generate unique nonce for each request', () => {
      const headers1 = signer.sign('POST', '/api/v1/payments');
      const headers2 = signer.sign('POST', '/api/v1/payments');
  ...
  describe('Incoming calls validate HMAC', () => {
    test('should validate correctly signed request', async () => {
      const method = 'POST';
      const path = '/api/v1/internal/payments/process';
    ...
    test('should validate correctly signed POST request with body', async () => {
      const method = 'POST';
      const path = '/api/v1/internal/payments/refund';
      const body = { paymentId: 'pay-123', amount: 5000, reason: 'customer_request' };
    ...
    test('should extract service name from validated request', async () => {
      ...
      const headers = toMutableHeaders(orderServiceSigner.sign('POST', '/api/v1/internal/payments'));
      const result = await validator.validate(headers, 'POST', '/api/v1/internal/payments');
  ...
  describe('Security - Replay attack prevention', () => {
    test('should reject request with reused nonce', async () => {
      const method = 'POST';
      const path = '/api/v1/internal/payments/secure';
      const headers = toMutableHeaders(signer.sign(method, path));
  ...
  describe('Security - Invalid signature detection', () => {
    test('should reject request with tampered signature', async () => {
      const method = 'POST';
      const path = '/api/v1/internal/payments/tampered';
      const headers = toMutableHeaders(signer.sign(method, path));
    ...
    test('should reject request signed with wrong secret', async () => {
      ...
      const headers = toMutableHeaders(wrongSigner.sign('POST', '/api/v1/internal/payments'));
      const result = await validator.validate(headers, 'POST', '/api/v1/internal/payments');
  ...
  describe('Security - Missing headers detection', () => {
    test('should reject request missing service header', () => {
      const headers = toMutableHeaders(signer.sign('POST', '/api/v1/internal/payments'));
