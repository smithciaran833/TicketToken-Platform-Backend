...
export class NFTQueueService {
  ...
  private setupProcessors() {
    this.mintQueue.process('mint-tickets', async (job) => {
      ...
      log.info({ paymentId: request.paymentId }, 'Processing NFT mint');
      ...
      try {
        ...
        log.error({ error }, 'Minting failed');
        throw error;
      }
    });
    ...
    this.batchQueue.process('batch-mint', async (job) => {
      ...
      log.info({ ticketCount: batch.ticketIds.length }, 'Processing batch mint');

      try {
        const result = await this.batchMintNFTs(batch);
        return result;
      } catch (error) {
        log.error({ error }, 'Batch minting failed');
        throw error;
      }
    });
  }
  ...
  private async mintNFTs(request: NFTMintRequest): Promise<any> {
    log.info({ ticketCount: request.ticketIds.length, blockchain: request.blockchain }, 'Minting NFTs');
  ...
  private async batchMintNFTs(batch: MintBatch): Promise<any> {
    log.info({ ticketCount: batch.ticketIds.length }, 'Batch minting NFTs');
    ...
    return {
      ...
      gasUsed: 0.0008 * batch.ticketIds.length
    };
  }
