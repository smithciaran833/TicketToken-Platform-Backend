/**
 * Internal Auth Middleware (New) - integration-service
 *
...
const INTERNAL_HMAC_SECRET = process.env.INTERNAL_HMAC_SECRET || process.env.INTERNAL_SERVICE_KEY;
const USE_NEW_HMAC = process.env.USE_NEW_HMAC === 'true';

const ALLOWED_SERVICES = new Set(
  (process.env.ALLOWED_INTERNAL_SERVICES || 'auth-service,event-service,ticket-service,payment-service,notification-service,venue-service,order-service')
    .split(',')
...
export interface InternalServiceClaimsNew {
  serviceName: string;
  isInternal: true;
...
declare module 'fastify' {
  interface FastifyRequest {
    internalServiceNew?: InternalServiceClaimsNew;
  }
}
