// @ts-nocheck
/**
 * HMAC Integration Tests - api-gateway
 *
...
describe('HMAC Integration Tests - api-gateway', () => {
  ...
  describe('Outgoing calls include HMAC headers', () => {
    test('should generate all required HMAC headers for GET request', () => {
      ...
      expect(headers[HMAC_HEADER_NAMES.SERVICE]).toBe('api-gateway');
  ...
  describe('Incoming calls validate HMAC', () => {
    test('should validate correctly signed request', async () => {
      const method = 'POST';
      const path = '/internal/gateway/route';
    ...
    test('should validate correctly signed POST request with body', async () => {
      const method = 'POST';
      const path = '/internal/gateway/proxy';
      const body = { serviceUrl: 'http://payment-service:3005', method: 'POST' };
    ...
    test('should extract service name from validated request', async () => {
      ...
      const headers = toMutableHeaders(authServiceSigner.sign('POST', '/internal/gateway/validate'));
      const result = await validator.validate(headers, 'POST', '/internal/gateway/validate');
  ...
  describe('Security - Replay attack prevention', () => {
    test('should reject request with reused nonce', async () => {
      const method = 'POST';
      const path = '/internal/gateway/secure';
      const headers = toMutableHeaders(signer.sign(method, path));
  ...
  describe('Security - Invalid signature detection', () => {
    test('should reject request with tampered signature', async () => {
      const method = 'POST';
      const path = '/internal/gateway/tampered';
      const headers = toMutableHeaders(signer.sign(method, path));
    ...
    test('should reject request signed with wrong secret', async () => {
      ...
      const headers = toMutableHeaders(wrongSigner.sign('POST', '/internal/gateway/hack'));
      const result = await validator.validate(headers, 'POST', '/internal/gateway/hack');
  ...
  describe('Security - Missing headers detection', () => {
    test('should reject request missing service header', () => {
      const headers = toMutableHeaders(signer.sign('POST', '/internal/gateway/test'));
