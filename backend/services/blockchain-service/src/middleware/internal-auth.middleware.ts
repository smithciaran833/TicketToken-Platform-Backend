/**
 * Internal Auth Middleware (New) - blockchain-service
 *
 ...
 *
 * FIXES:
 * - Renamed x-timestamp → x-internal-timestamp
 * - Added nonce support for replay attack prevention
 * - Standardized on shared HmacValidator
 */
...
const ALLOWED_SERVICES = new Set(
  (process.env.ALLOWED_INTERNAL_SERVICES || 'payment-service,ticket-service,order-service,minting-service,transfer-service,marketplace-service,api-gateway')
    .split(',')
...
export async function internalAuthMiddlewareNew(
  ...
  try {
    // Extract HMAC headers (standardized format)
    const headers = {
