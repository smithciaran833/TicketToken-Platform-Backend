// @ts-nocheck
/**
 * HMAC Integration Tests - event-service
 *
 ...
 * - X-Service-Token (base64 JSON with embedded signature)
 * - X-API-Key (for API key authentication)
 *
 * These tests verify the new standardized HMAC implementation.
 */
...
describe('HMAC Integration Tests - event-service', () => {
  ...
  describe('Outgoing calls include HMAC headers', () => {
    test('should generate all required HMAC headers for GET request', () => {
      const headers = signer.sign('GET', '/api/v1/events/list');

      expect(headers[HMAC_HEADER_NAMES.SERVICE]).toBe('event-service');
    ...
    test('should generate body hash header for POST request with body', () => {
      const body = { name: 'Concert 2024', venueId: 'venue-123', startsAt: '2024-12-31T20:00:00Z' };
      const headers = signer.sign('POST', '/api/v1/events/create', body);
    ...
    test('should generate unique nonce for each request', () => {
      const headers1 = signer.sign('POST', '/api/v1/events/publish');
      const headers2 = signer.sign('POST', '/api/v1/events/publish');
  ...
  describe('Incoming calls validate HMAC', () => {
    test('should validate correctly signed request', async () => {
      const method = 'GET';
      const path = '/internal/events/event-123';
    ...
    test('should validate correctly signed POST request with body', async () => {
      const method = 'POST';
      const path = '/internal/events/batch-lookup';
      const body = { eventIds: ['event-1', 'event-2', 'event-3'] };
    ...
    test('should extract service name from validated request', async () => {
      const venueServiceSigner = createHmacSigner({
        secret: testSecret,
        serviceName: 'venue-service',
      });

      const headers = toMutableHeaders(venueServiceSigner.sign('GET', '/internal/events/by-venue/venue-123'));
      const result = await validator.validate(headers, 'GET', '/internal/events/by-venue/venue-123');
  ...
  describe('Security - Replay attack prevention', () => {
    test('should reject request with reused nonce', async () => {
      const method = 'POST';
      const path = '/internal/events/secure';
      const headers = toMutableHeaders(signer.sign(method, path));
  ...
  describe('Security - Invalid signature detection', () => {
    test('should reject request with tampered signature', async () => {
      const method = 'POST';
      const path = '/internal/events/tampered';
      const headers = toMutableHeaders(signer.sign(method, path));
    ...
    test('should reject request signed with wrong secret', async () => {
      ...
      const headers = toMutableHeaders(wrongSigner.sign('POST', '/internal/events/hack'));
      const result = await validator.validate(headers, 'POST', '/internal/events/hack');
  ...
  describe('Security - Missing headers detection', () => {
    test('should reject request missing service header', () => {
      const headers = toMutableHeaders(signer.sign('POST', '/internal/events/test'));
