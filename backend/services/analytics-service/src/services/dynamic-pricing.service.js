...
const demand_tracker_service_1 = require("./demand-tracker.service");
const logger_1 = require("../utils/logger");
class DynamicPricingService {
    static instance;
    log = logger_1.logger.child({ component: 'DynamicPricingService' });
    static getInstance() {
        if (!this.instance) {
    ...
    async calculateOptimalPrice(eventId, basePrice, rules) {
        if (!rules.enabled) {
            return {
                eventId,
    ...
    calculateConfidence(demand, reasoningCount) {
        let confidence = 0.5;
        if (demand.salesVelocity > 0)
            confidence += 0.1;
        if (demand.ticketsSold > 10)
            confidence += 0.1;
        if (demand.priceElasticity !== 1.0)
            confidence += 0.1;
        if (reasoningCount >= 3)
            confidence += 0.1;
        return Math.min(1.0, confidence);
    }
    async getVenuePricingRules(venueId) {
        try {
    ...
    async applyPriceChange(eventId, newPrice, reason) {
        try {
            const db = (0, database_1.getDb)();
...
exports.dynamicPricingService = DynamicPricingService.getInstance();
