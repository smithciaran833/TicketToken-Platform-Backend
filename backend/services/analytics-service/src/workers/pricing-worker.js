...
class PricingWorker {
    ...
    async start() {
        this.isRunning = true;
        logger_1.logger.info('🚀 Pricing worker started');
        while (this.isRunning) {
            try {
                await this.processEvents();
            }
            catch (error) {
                logger_1.logger.error('Pricing worker error:', error);
            }
    ...
    stop() {
        ...
        logger_1.logger.info('🛑 Pricing worker stopped');
    }
    async processEvents() {
    ...
    `);
        logger_1.logger.info(`Processing ${result.rows.length} events with dynamic pricing`);
    ...
    async processEvent(event) {
        ...
        const recommendation = await dynamic_pricing_service_1.dynamicPricingService.calculateOptimalPrice(event.id, event.price_cents, rules);
        logger_1.logger.info(`Event ${event.id}:`);
        logger_1.logger.info(`  Current: $${(event.price_cents / 100).toFixed(2)}`);
        logger_1.logger.info(`  Recommended: $${(recommendation.recommendedPrice / 100).toFixed(2)}`);
        logger_1.logger.info(`  Demand Score: ${recommendation.demandScore}/100`);
        logger_1.logger.info(`  Confidence: ${(recommendation.confidence * 100).toFixed(0)}%`);
        ...
        if (changePercent < 0.05) {
            logger_1.logger.info(`  ⏭️  Change too small (${(changePercent * 100).toFixed(1)}%), skipping`);
            return;
        }
        const db = (0, database_1.getDb)();
      ...
      `, [event.id, event.price_cents, recommendation.recommendedPrice, recommendation.confidence, JSON.stringify(recommendation.reasoning), recommendation.demandScore, recommendation.recommendedPrice, recommendation.confidence, JSON.stringify(recommendation.reasoning), recommendation.demandScore]);
            logger_1.logger.info(`  ⏸️  Awaiting approval`);
        }
        else {
            await dynamic_pricing_service_1.dynamicPricingService.applyPriceChange(event.id, recommendation.recommendedPrice, `Auto-adjusted: ${recommendation.reasoning.join(', ')}`);
            logger_1.logger.info(`  ✅ Price updated automatically`);
        }
    }
    sleep(ms) {
...
exports.PricingWorker = PricingWorker;
//# sourceMappingURL=pricing-worker.js.map