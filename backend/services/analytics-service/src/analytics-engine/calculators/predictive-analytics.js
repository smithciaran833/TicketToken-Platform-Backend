...
class PredictiveAnalytics {
    ...
    async predictTicketDemand(venueId, eventDate, eventType) {
    ...
    `, [venueId]);
        ...
        const similarEvents = historicalEvents.rows.filter((event) => event.day_of_week === targetDayOfWeek &&
            Math.abs(event.month - targetMonth) <= 1);
        if (similarEvents.length === 0) {
            return { error: 'No similar events found' };
        }
        const avgTickets = similarEvents.reduce((sum, e) => sum + e.tickets_sold, 0) / similarEvents.length;
        const avgCapacity = similarEvents.reduce((sum, e) => sum + parseFloat(e.capacity_sold_percentage), 0) / similarEvents.length;
        const avgRevenue = similarEvents.reduce((sum, e) => sum + parseFloat(e.revenue), 0) / similarEvents.length;
    ...
    async predictSeasonalTrends(venueId) {
    ...
    async predictOptimalPricing(venueId, eventType) {
    ...
    calculateConfidence(sampleSize, trend) {
        if (sampleSize < 5)
            return 'Low';
        if (sampleSize < 20)
            return 'Medium';
    ...
    categorizeSeasonality(monthAvg, allMonths) {
    ...
    generatePricingRecommendation(optimalBand, elasticity) {
