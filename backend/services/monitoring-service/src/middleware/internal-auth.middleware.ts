/**
 * Internal Auth Middleware (New) - monitoring-service
 *
 ...
 * This is a new implementation to enable S2S authentication for monitoring endpoints.
 */
...
const ALLOWED_SERVICES = new Set(
  (process.env.ALLOWED_INTERNAL_SERVICES || 'api-gateway,admin-service')
    .split(',')
...
// Signer for outgoing health check requests
...
/**
 * Generate HMAC headers for outgoing health check requests
 * This wraps bare axios calls with proper S2S authentication
 */
export function generateInternalAuthHeaders(
  ...
  if (!USE_NEW_HMAC || !hmacSigner) {
    // Return empty headers if HMAC is disabled
    return {};
  }
