/**
 * Internal Auth Middleware - file-service
 *
...
const ALLOWED_SERVICES = new Set(
  (process.env.ALLOWED_INTERNAL_SERVICES || 'ticket-service,order-service,event-service,venue-service')
    .split(',')
...
export async function internalAuthMiddleware(
  ...
  if (!USE_NEW_HMAC) {
    ...
    // Allow request but mark as not internally authenticated
    return;
  }
  ...
  try {
    ...
    // Validate HMAC signature using shared library - correct API
    ...
    // Backward compatibility - also set as string
    (request as any).internalServiceName = serviceName;
...
/**
 * Optional internal auth - validates if headers present, otherwise continues
 */
export async function optionalInternalAuth(
  request: FastifyRequest,
  reply: FastifyReply
): Promise<void> {
  const hasInternalHeaders = request.headers['x-internal-service'] &&
                             request.headers['x-internal-signature'];

  if (hasInternalHeaders) {
    return internalAuthMiddleware(request, reply);
  }
  // Continue without internal auth validation
}
