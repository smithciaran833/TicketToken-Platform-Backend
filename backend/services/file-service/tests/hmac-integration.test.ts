 ...
 * file-service receives internal calls from compliance-service, ticket-service, etc.
 */
...
describe('HMAC Integration Tests - file-service', () => {
  ...
  let complianceServiceSigner: HmacSigner;
  ...
  beforeAll(() => {
    ...
    // Create signers for services that call file-service
    complianceServiceSigner = createHmacSigner({
      secret: testSecret,
  ...
  describe('Outgoing calls include HMAC headers', () => {
    test('should generate all required HMAC headers for GET request', () => {
      ...
      const headers = signer.sign('GET', '/api/v1/internal/files/file-123');
    ...
    test('should generate body hash header for POST request with body', () => {
      ...
      const body = { userId: 'user-123', fileName: 'document.pdf' };
      const headers = signer.sign('POST', '/api/v1/internal/files', body);
    ...
    test('should generate unique nonce for each request', () => {
      ...
      const headers1 = signer.sign('GET', '/api/v1/internal/files');
      const headers2 = signer.sign('GET', '/api/v1/internal/files');
  ...
  // Primary focus: file-service receives calls from compliance-service, ticket-service
  // =========================================================================

  describe('Incoming calls validate HMAC', () => {
    test('should validate request from compliance-service', async () => {
      const method = 'GET';
      const path = '/api/v1/internal/users/user-123/files';
      const headers = toMutableHeaders(complianceServiceSigner.sign(method, path));

      const result = await validator.validate(headers, method, path);

      expect(result.valid).toBe(true);
      expect(result.serviceName).toBe('compliance-service');
    });

    test('should validate request from ticket-service', async () => {
      const method = 'GET';
      const path = '/api/v1/internal/files/file-456';
      const headers = toMutableHeaders(ticketServiceSigner.sign(method, path));
    ...
    test('should validate POST request with body', async () => {
      const method = 'POST';
      const path = '/api/v1/internal/files/process';
      const body = { fileId: 'file-123', action: 'compress' };
      const headers = toMutableHeaders(complianceServiceSigner.sign(method, path, body));
    ...
    test('should extract correct service name from different services', async () => {
      ...
      const headers = toMutableHeaders(orderServiceSigner.sign('GET', '/api/v1/internal/files'));
      const result = await validator.validate(headers, 'GET', '/api/v1/internal/files');
  ...
  describe('Security - Replay attack prevention', () => {
    test('should reject request with reused nonce', async () => {
      const method = 'GET';
      const path = '/api/v1/internal/files/file-replay-test';
      const headers = toMutableHeaders(complianceServiceSigner.sign(method, path));
  ...
  describe('Security - Invalid signature detection', () => {
    test('should reject request with tampered signature', async () => {
      const method = 'GET';
      const path = '/api/v1/internal/files/tampered';
      const headers = toMutableHeaders(complianceServiceSigner.sign(method, path));
    ...
    test('should reject request signed with wrong secret', async () => {
      ...
      const headers = toMutableHeaders(wrongSigner.sign('GET', '/api/v1/internal/files'));
      const result = await validator.validate(headers, 'GET', '/api/v1/internal/files');

      expect(result.valid).toBe(false);
    });

    test('should reject request with modified path', async () => {
      const method = 'GET';
      const originalPath = '/api/v1/internal/files/file-123';
      const modifiedPath = '/api/v1/internal/files/file-999';
      const headers = toMutableHeaders(complianceServiceSigner.sign(method, originalPath));

      // Try to use signature for different path
      const result = await validator.validate(headers, method, modifiedPath);
  ...
  describe('Security - Missing headers detection', () => {
    test('should reject request missing service header', () => {
      const headers = toMutableHeaders(complianceServiceSigner.sign('GET', '/api/v1/internal/files'));
