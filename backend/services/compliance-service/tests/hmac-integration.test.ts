// @ts-nocheck
/**
 * HMAC Integration Tests - compliance-service
 *
...
describe('HMAC Integration Tests - compliance-service', () => {
  ...
  describe('Outgoing calls include HMAC headers', () => {
    test('should generate all required HMAC headers for GET request', () => {
      const headers = signer.sign('GET', '/api/v1/compliance/ofac/status');

      expect(headers[HMAC_HEADER_NAMES.SERVICE]).toBe('compliance-service');
    ...
    test('should generate body hash header for POST request with body', () => {
      const body = { userId: 'user-123', screeningType: 'ofac', data: { name: 'John Doe' } };
      const headers = signer.sign('POST', '/api/v1/compliance/ofac/screen', body);
    ...
    test('should generate unique nonce for each request', () => {
      const headers1 = signer.sign('POST', '/api/v1/compliance/risk/assess');
      const headers2 = signer.sign('POST', '/api/v1/compliance/risk/assess');
  ...
  describe('Incoming calls validate HMAC', () => {
    test('should validate correctly signed request', async () => {
      const method = 'POST';
      const path = '/internal/compliance/ofac/screen';
    ...
    test('should validate correctly signed POST request with body', async () => {
      const method = 'POST';
      const path = '/internal/compliance/gdpr/export';
      const body = { userId: 'user-456', exportType: 'full', format: 'json' };
    ...
    test('should extract service name from validated request', async () => {
      ...
      const headers = toMutableHeaders(paymentServiceSigner.sign('POST', '/internal/compliance/aml/check'));
      const result = await validator.validate(headers, 'POST', '/internal/compliance/aml/check');
  ...
  describe('Security - Replay attack prevention', () => {
    test('should reject request with reused nonce', async () => {
      const method = 'POST';
      const path = '/internal/compliance/secure';
      const headers = toMutableHeaders(signer.sign(method, path));
  ...
  describe('Security - Invalid signature detection', () => {
    test('should reject request with tampered signature', async () => {
      const method = 'POST';
      const path = '/internal/compliance/tampered';
      const headers = toMutableHeaders(signer.sign(method, path));
    ...
    test('should reject request signed with wrong secret', async () => {
      ...
      const headers = toMutableHeaders(wrongSigner.sign('POST', '/internal/compliance/hack'));
      const result = await validator.validate(headers, 'POST', '/internal/compliance/hack');
  ...
  describe('Security - Missing headers detection', () => {
    test('should reject request missing service header', () => {
      const headers = toMutableHeaders(signer.sign('POST', '/internal/compliance/test'));
