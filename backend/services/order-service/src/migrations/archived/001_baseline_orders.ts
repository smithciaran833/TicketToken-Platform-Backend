...
/**
 * Baseline Orders Migration
 * 
 * HIGH: Note on CREATE INDEX CONCURRENTLY
 * - CONCURRENTLY cannot be used inside a transaction (Knex migrations run in transactions by default)
 * - For initial table creation, regular CREATE INDEX is acceptable since tables are empty
 * - For future index additions on tables with data, create a separate migration file
 *   that uses knex.raw with { transaction: false } or a dedicated migration script
 * 
 * Example for future concurrent index additions:
 *   // In a new migration file with exports.config = { transaction: false }
 *   await knex.raw('CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_name ON table(column)');
 */

export async function up(knex: Knex): Promise<void> {
  // CRITICAL: Set timeouts to prevent migrations from blocking all queries indefinitely
  ...
  // HIGH: Index for dispute lookups
  await knex.raw(`CREATE INDEX IF NOT EXISTS idx_orders_dispute_id ON orders (dispute_id) WHERE dispute_id IS NOT NULL`);
  await knex.raw(`CREATE INDEX IF NOT EXISTS idx_orders_has_dispute ON orders (tenant_id, has_dispute) WHERE has_dispute = TRUE`);
  ...
  await knex.raw('ALTER TABLE order_disputes ENABLE ROW LEVEL SECURITY');
  ...
  await knex.raw(`CREATE POLICY order_disputes_tenant_isolation ON order_disputes FOR ALL USING (tenant_id = current_setting('app.current_tenant')::uuid)`);
...
export async function down(knex: Knex): Promise<void> {
  ...
  await knex.raw('DROP POLICY IF EXISTS order_disputes_tenant_isolation ON order_disputes');
  ...
  await knex.raw('ALTER TABLE order_disputes DISABLE ROW LEVEL SECURITY');
