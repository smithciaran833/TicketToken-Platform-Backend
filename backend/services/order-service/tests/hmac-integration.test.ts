// @ts-nocheck
/**
 * HMAC Integration Tests - order-service
 *
...
describe('HMAC Integration Tests - order-service', () => {
  ...
  describe('Outgoing calls include HMAC headers', () => {
    test('should generate all required HMAC headers for GET request', () => {
      ...
      expect(headers[HMAC_HEADER_NAMES.SERVICE]).toBe('order-service');
    ...
    test('should generate body hash header for POST request with body', () => {
      const body = { userId: 'user-123', orderId: 'order-456', items: [{ ticketId: 't-1' }] };
      const headers = signer.sign('POST', '/api/v1/orders/create', body);
    ...
    test('should generate unique nonce for each request', () => {
      const headers1 = signer.sign('POST', '/api/v1/orders');
      const headers2 = signer.sign('POST', '/api/v1/orders');
  ...
  describe('Incoming calls validate HMAC', () => {
    test('should validate correctly signed request', async () => {
      const method = 'POST';
      const path = '/api/v1/internal/orders/update';
    ...
    test('should validate correctly signed POST request with body', async () => {
      const method = 'POST';
      const path = '/api/v1/internal/orders/payment-webhook';
      const body = { orderId: 'order-123', status: 'paid', amount: 9999 };
    ...
    test('should extract service name from validated request', async () => {
      ...
      const headers = toMutableHeaders(paymentServiceSigner.sign('POST', '/api/v1/internal/orders'));
      const result = await validator.validate(headers, 'POST', '/api/v1/internal/orders');
  ...
  describe('Security - Replay attack prevention', () => {
    test('should reject request with reused nonce', async () => {
      const method = 'POST';
      const path = '/api/v1/internal/orders/secure';
      const headers = toMutableHeaders(signer.sign(method, path));
  ...
  describe('Security - Invalid signature detection', () => {
    test('should reject request with tampered signature', async () => {
      const method = 'POST';
      const path = '/api/v1/internal/orders/tampered';
      const headers = toMutableHeaders(signer.sign(method, path));
    ...
    test('should reject request signed with wrong secret', async () => {
      ...
      const headers = toMutableHeaders(wrongSigner.sign('POST', '/api/v1/internal/orders'));
      const result = await validator.validate(headers, 'POST', '/api/v1/internal/orders');
  ...
  describe('Security - Missing headers detection', () => {
    test('should reject request missing service header', () => {
      const headers = toMutableHeaders(signer.sign('POST', '/api/v1/internal/orders'));
