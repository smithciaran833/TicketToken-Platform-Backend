// @ts-nocheck
/**
 * HMAC Integration Tests - ticket-service
 *
 * Tests for HMAC authentication on service-to-service communication.
 * Phase A HMAC Standardization - Week 3
 *
 * CRITICAL SECURITY: This service previously had a temp-signature bypass vulnerability.
 * These tests verify that ALL requests require valid HMAC authentication.
 * NO BYPASSES ARE ALLOWED.
 */
...
describe('HMAC Integration Tests - ticket-service', () => {
  ...
  describe('Outgoing calls include HMAC headers', () => {
    test('should generate all required HMAC headers for GET request', () => {
      const headers = signer.sign('GET', '/api/v1/tickets/status');

      expect(headers[HMAC_HEADER_NAMES.SERVICE]).toBe('ticket-service');
    ...
    test('should generate body hash header for POST request with body', () => {
      const body = { ticketId: 'ticket-123', eventId: 'event-456', status: 'SOLD' };
      const headers = signer.sign('POST', '/api/v1/tickets/create', body);
    ...
    test('should generate unique nonce for each request', () => {
      const headers1 = signer.sign('POST', '/api/v1/tickets/transfer');
      const headers2 = signer.sign('POST', '/api/v1/tickets/transfer');
  ...
  describe('Incoming calls validate HMAC', () => {
    test('should validate correctly signed request', async () => {
      const method = 'GET';
      const path = '/internal/tickets/ticket-123/status';
    ...
    test('should validate correctly signed POST request with body', async () => {
      const method = 'POST';
      const path = '/internal/tickets/cancel-batch';
      const body = { ticketIds: ['t1', 't2'], reason: 'refund', refundId: 'ref-123' };
    ...
    test('should extract service name from validated request', async () => {
      ...
      const headers = toMutableHeaders(orderServiceSigner.sign('GET', '/internal/tickets/status'));
      const result = await validator.validate(headers, 'GET', '/internal/tickets/status');
  ...
  describe('Security - No bypass allowed (CRITICAL FIX)', () => {
    test('should reject request with "temp-signature" - bypass removed', async () => {
      // This test verifies the temp-signature bypass has been removed
      // The old code would accept any request with signature "temp-signature"
      const headers = {
        [HMAC_HEADER_NAMES.SERVICE]: 'attacker-service',
        [HMAC_HEADER_NAMES.TIMESTAMP]: Date.now().toString(),
        [HMAC_HEADER_NAMES.NONCE]: 'fake-nonce',
        [HMAC_HEADER_NAMES.SIGNATURE]: 'temp-signature', // The old bypass value
      };

      const result = await validator.validate(headers, 'POST', '/internal/tickets/hack');

      // SECURITY: Must be rejected - no bypass allowed
  ...
  describe('Security - Invalid signature detection', () => {
    test('should reject request with tampered signature', async () => {
      const method = 'POST';
      const path = '/internal/tickets/tampered';
      const headers = toMutableHeaders(signer.sign(method, path));
    ...
    test('should reject request signed with wrong secret', async () => {
      ...
      const headers = toMutableHeaders(wrongSigner.sign('POST', '/internal/tickets/hack'));
      const result = await validator.validate(headers, 'POST', '/internal/tickets/hack');
  ...
  describe('Security - Missing headers detection', () => {
    test('should reject request missing service header', () => {
      const headers = toMutableHeaders(signer.sign('POST', '/internal/tickets/test'));
