...
export default async function ticketRoutes(fastify: FastifyInstance) {
  ...
  fastify.post('/types', {
    preHandler: [rateLimiters.write, authMiddleware, requireRole(['admin', 'venue_manager']), validate(ticketSchemas.createTicketType)]
  ...
  fastify.get('/events/:eventId/types', {
    preHandler: [rateLimiters.read, tenantMiddleware]
  ...
  fastify.put('/types/:id', {
    preHandler: [rateLimiters.write, authMiddleware, requireRole(['admin', 'venue_manager'])]
  }, (request, reply) => ticketController.updateTicketType(request, reply));
  // Get single ticket by ID (requires authentication + ownership)
